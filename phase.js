...
(function () {
  ...
  //   Phase 2 = 论坛登录 + 聊天室 + 档案馆深层 + 矛盾点(何宏宇/俞丽) + 停职公告 + 广播站
  //   Phase 3 = 身份翻转(糖醋排骨) + 校友旧刊/凯撒解密 + 秘密基地 + 日记 + 校友聚集/招生咨询(终局)
  // 注：天台、档案馆的「深层内容」由各自谜题进度门禁（密码锁/授权口令），本阶段仅控制其首次可被发现/搜索。
  // 注：资源下载(downloads.html)为常驻导航页，已从 PHASE_OF 移除 → 永远可开（阶段0）；其内「生命教育PDF」深层内容仍由页内 data-phase="3" 控制，第三阶段才显示。
  var PHASE_OF = {
    ...
    // ── 第二阶段：论坛 + 聊天室 + 档案馆深层 + 矛盾点 + 停职/广播 ──
    "page11-luntan.html": 2,          // 校园论坛（进入档案馆取得纪佑泽后登录）
    ...
    "page7-xiaoyou.html": 3,          // 校友聚集（2006届红色标记，终局相关，导航页深层）
    "page10-zhaosheng.html": 3        // 招生咨询（祈愿对话/结局，导航页深层）
  };
